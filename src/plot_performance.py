...
fig, axs = plt.subplots(3, 3, sharex='col', figsize=(15, 10))
...
for folder in list_of_folders:
    ...
    for filename in all_files:
        ...
        # plot reward
        axs[2,j].plot(mean_reward, linestyle='-.', color=list_colors[i], lw=1)
        axs[2,j].fill_between(range(len(mean_reward)),
    ...
    axs[2,j].hlines(y=opt_reward, xmin=0, xmax=XMAX, colors='#7f7f7f', linestyles='--', lw=1)
    axs[2,j].hlines(y=safe_reward, xmin=0, xmax=XMAX, colors='#7f7f7f', linestyles=':', lw=1)
    axs[2,j].hlines(y=fast_reward, xmin=0, xmax=XMAX, colors='#7f7f7f', linestyles=':', lw=1)
    axs[2,j].text(text_pos_x, safe_reward - eps_shift_down, 'safe', color='#7f7f7f')
    axs[2,j].text(text_pos_x, fast_reward + eps_shift_up, 'fast', color='#7f7f7f')
    #axs[2,j].set(xlabel='rounds', ylabel='reward')
    axs[2, j].set(xlabel='rounds')
...
rows = ['{}'.format(row) for row in ['main regret', 'consumption', 'reward']]
...
plt.savefig('av_reward_cost' + '.png', bbox_inches='tight')
plt.show()
